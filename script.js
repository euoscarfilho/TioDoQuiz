...
import { settings, loadSettings, saveSettings } from './state.js';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('fullscreenchange', () => {
        // Importa o estado de gravação e a função de parar dinamicamente
        import('./state.js').then(state => {
            if (!document.fullscreenElement && state.isRecording) {
                ...
                stopRecordingAndDownload(); // Para a gravação se sair do fullscreen
            }
        });
    });

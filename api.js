...
// --- CHAVES DE API ---
// Chave para a API de TEXTO (Gemini da Google)
const GOOGLE_API_KEY = "AIzaSyATTawnL9AaeDms3b6xkGyXGgYrvhbirFI";
// Chave para a API de ÁUDIO (Eleven Labs)
const ELEVENLABS_API_KEY = "sk_5d54150b3f62fabc2a4478abf8dfdd38ccf55c9d7e5a4df3";
// ID da Voz (Voz "Antoni" - PT-BR)
const ELEVENLABS_VOICE_ID = "NOpBlnGInO9m6vDvFkFC"; 

// --- GERAÇÃO DE ÁUDIO (NOVO: Eleven Labs) ---

/**
 * Busca um arquivo de áudio da API Eleven Labs TTS.
 * Retorna um Blob URL para o áudio.
 */
async function fetchAudio(text, speed = 1.20) { // Velocidade 1.20 como padrão
    ...
    try {
        // Delay para evitar rate limits (boa prática)
        ...
        if (!response.ok) {
            if (response.status === 429) {
                ...
                return fetchAudio(text, speed); // Tenta novamente
            }
        ...
        // A resposta é o próprio arquivo de áudio
...
export async function generateQuizAndScriptFromTheme() {
    ...
    try {
        ...
        if (!response.ok) throw new Error(`Erro na API Gemini: ${response.status} ${response.statusText}`);
    ...
    for (let i = 0; i < quizData.length; i++) {
        ...
        // Pergunta
        const questionText = `${item.question}`; // Texto limpo para narração
        ...
        textsToGenerate.push({ id: `q${i}_r`, text: revealText }); // Áudio da Resposta
...
export async function handleGenerateContentClick() {
    if (!ELEVENLABS_API_KEY || ELEVENLABS_API_KEY.includes("COLE_SUA_CHAVE")) {
         alert("Por favor, adicione sua chave de API do Eleven Labs no arquivo api.js");
         return;
    }
    ...
    try {
        ...
        C.generationSuccessMsg.textContent = `Quiz gerado! (${result.credits})`; // Mostra os créditos
